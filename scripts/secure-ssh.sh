...
if ! id -u "${ADMIN_USER}" &>/dev/null; then
  echo "[1/5] Creating user '${ADMIN_USER}'..."
  useradd -m -s /bin/bash "${ADMIN_USER}"
else
  echo "[1/5] User '${ADMIN_USER}' already exists. Skipping creation."
fi

# 2) Ensure existing ec2-user key is copied to admin
echo "[2/5] Deploying ec2-user public key to admin..."
EC2_AUTH="/home/ec2-user/.ssh/authorized_keys"
ADMIN_SSH_DIR="/home/${ADMIN_USER}/.ssh"
ADMIN_AUTH="${ADMIN_SSH_DIR}/authorized_keys"

# Verify ec2-user key exists
if [ ! -f "${EC2_AUTH}" ]; then
  echo "Error: ec2-user authorized_keys not found at ${EC2_AUTH}" >&2
  exit 1
fi

# Create admin .ssh and copy
...
echo " → Authorized key for admin deployed."

# 3) Install & configure SSH
echo "[3/5] Installing and configuring OpenSSH..."
...
echo "[5/5] SSH and Fail2Ban setup complete. Use your existing private key to login as 'admin'."