...
echo "2. Serveur de Monitoring: http://$MONITORING_IP:9090 (monitoring|pass)"
echo "3. Serveur de Uptime Kuma: http://$MONITORING_IP:3001/dashboard (admin|admin123)"
...
echo " - Panel Fail2ban : sudo fail2ban-status"
echo " - Vérifier les mises à jour : journalctl -u dnf-automatic.service --no-pager"
echo " - Backups automatique : ssh backup@$BACKUP_IP 'crontab -l' (pxmiXvkEte808X) et ls -lR /backups"